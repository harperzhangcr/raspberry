...
DASHBOARD_OUTPUT_PATH = os.environ.get(
    ...
    os.path.join(ARTIFACTS_DIR, "创新管理委员会会议材料.html")
)
...
def update_slide_1_2(html_text: str, meta: dict, metrics: dict, metric_table: list[list[str]]) -> str:
    ...
    replacements = {
        r'(<span data-dashboard-field="meeting-title">).*?(</span>)': rf'\g<1>{meta["year"]}年第{meta["meeting_no"]}次创新管理委员会\2',
        ...
        r'(<p\b[^>]*data-dashboard-field="report截止日"[^>]*>).*?(</p>)': rf'\g<1>截至 {meta["cutoff"]}\2',
