...
- 2026-06-26 audit note: source workbook `贷后风险等级卡+处置动作卡0625_动作角色编码版.xlsx` was updated after the generated dual-card table. Current action-code audit shows 296 action rows, 70 used action codes, no missing/unused action codes, and no unified-action/action-category mismatches. Role issues remain for `ACT035 车辆处置` missing role/role code in four commercial conventional sheets, plus passenger abnormal `ACT131/ACT132/ACT102` role-code convention mismatches.
